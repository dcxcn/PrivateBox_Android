 ...
 * mht转html格式文件
 *
 * @修改人 FQQ(www.first.top)
 * @版本号 1.0.0 @修改日期： 2016年11月02日 上午10:48:20
 * @see [相关类/方法]
 * @since [产品/模块版本]
 */
public class Mht2HtmlUtil {
    ...
    public static void main(String[] args) {
        Mht2HtmlUtil.mht2html("d:\\51job_111.mht", "d:\\test2222222222.htm");
    }
    /**
     * 将 mht文件转换成 html文件
     *
    ...
    public static void mht2html(String s_SrcMht, String s_DescHtml) {
        try {
            ...
            if (content instanceof Multipart) {
                ...
                // 获取html文件的内容
                MimeBodyPart bp_html = getMhtHtmlPart(mp);
                // 获取mht文件内容代码的编码
                String strEncodng = getEncoding(bp_html);
                // 获取mht文件的内容
                String strText = getHtmlText(bp_html, strEncodng);
                if (strText == null)
                    return;
                ...
                if (mp.getCount() > 1) {
                    ...
                    parent = new File(s_DescHtml.replace(".html",".files"));
    ...
    public static MimeBodyPart getMhtHtmlPart(MimeMultipart mp){
        try {
            for (int i = 0; i < mp.getCount(); i++) {
                MimeBodyPart bp = (MimeBodyPart) mp.getBodyPart(i);
                if(  bp.getContentType().indexOf("htm")!=-1){
                    return bp;
                }
            }
        }catch (Exception exp){}
